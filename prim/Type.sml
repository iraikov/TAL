...
and  tsubst  T (Tvar a)            = (case Env.find (a, T) of 
   ...
   | tsubst  T (Arrow (a,ts,u))    =
     let
	 val ss  = List.tabulate (length a, fn(x) => Symbol.freshPrefix "primty")
...
fun unify (Nil, Nil)           = true
  ...
  | unify (Arrow (a,u,t), Arrow (a',u',t')) = 
    (if (List.length a) = (List.length a') 
     then (let val ss  = List.tabulate (length a, fn(x) => Symbol.freshPrefix "primty")
...
fun partial (Arrow([],[t1],t2),vt :: vts) = 
    if unify (t1,vt) then partial (t2,vts) else raise TypeError
  | partial (t1,[])  = t1
  | partial (t1,vts) = raise TypeError
...
fun prim_pptyp ty = Pretty.printf TextIO.stdOut (Pretty.format 60 (prim_pptyp' ty))
