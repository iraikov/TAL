...
and  tsubst  T (Tvar a)            = (case Env.find (a, T) of 
   ...
   | tsubst  T (Arrow (a,ts,u))    =
     let
	 val ss  = List.tabulate (length a, fn(x) => Symbol.freshPrefix "closty")
   ...
   | tsubst T (Record (ss,env))   = Record (ss,Env.map (fn(t) => tsubst T t) env)
   | tsubst T (Exists (a,t))      = 
     let 
	 val s = Symbol.freshPrefix "closty" 
	 val T' = compose ([a],[s],T)
     in 
	 Exists(s,tsubst T' t)
     end
...
fun unify (Nil, Nil)           = true
  ...
  | unify (Record (_,u), Record (_,v)) = 
    (ListPair.all (fn(u,v) => unify (u,v))
		  (Env.listItems u, Env.listItems v))
  | unify (Arrow (a,u,t), Arrow (a',u',t')) = 
    (if (List.length a) = (List.length a') 
     then (let val ss  = List.tabulate (length a, fn(x) => Symbol.freshPrefix "closty")
  ...
  | unify (Exists (s,t), Exists (s',t')) = 
    let val p  =  Symbol.freshPrefix "closty"
		  ...
		  | (Record (ss,env)) => 
		    group (connect (text "Record", 
				    block {l=brace L, r=brace R} 2
					  (list 2 (ppField,comma) (map (fn(s) => (s,Env.look(s,env))) ss))))
...
fun clos_pptyp ty = Pretty.printf TextIO.stdOut (Pretty.format 60 (clos_pptyp' ty))
